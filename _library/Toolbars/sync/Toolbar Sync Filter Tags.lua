-- @description Velocitone: Toolbar Sync Filter Tags
-- @author chrislundeen
-- @website https://github.com/chrislundeen/
-- @noindex
-- @version 1.0.0
-- @changelog
--   Initial Velocitone release. Syncs toolbar button states with active tag filters.
...
--------------------------------------------
local library_files = {
  'Toolbars/helpers/Get Filter Tags.lua'
}
...
function SyncVelocitoneToolbarFilterTags()

  local totalTags = getFilterTags()
  if (totalTags.isChanged) then -- only do this looping if filter text changed
    ...
    for index, tagname in ipairs(velocitone_config.tags) do
      ...
      if (isFound) then
        reaper.SetToggleCommandState(0, reaper.NamedCommandLookup(velocitone_config.scriptCommandIDs.tagFilter[tagname]),
          1)
      else
        reaper.SetToggleCommandState(0, reaper.NamedCommandLookup(velocitone_config.scriptCommandIDs.tagFilter[tagname]),
          -1)
